...
function _getLocale(): string {
  const defaultLocale: string = _SUPPORTED_LOCALES_[0]
...
function _setLocale(locale?: string | null): void {
  localStorage.removeItem(_LOCALEKEY_)
  if ((locale ?? '').length > 0) {
    localStorage.setItem(_LOCALEKEY_, locale ?? 'navigator')
  }
}

function _addTranslation(locale: string, translation?: i18nJSON): void {
  if (!(locale ?? '').length) {
    return
  }
...
function _translate(message: string, params?: i18nParameters): string {
  ...
  var output: string | null = translation[message] ?? message ?? ''
  if (output?.indexOf('|') > -1) {
